...
public:
  t_java_generator(t_program* program,
    ...
    for( iter = parsed_options.begin(); iter != parsed_options.end(); ++iter) {
      ...
      } else if( iter->first.compare("enum_containers") == 0) {
        enum_containers_ = true;
...
void t_java_generator::generate_deep_copy_non_container(ofstream& out,
                                                        ...
                                                        t_type* type) {
  (void)dest_name;
  t_type* tt = get_true_type(type);
  if (tt->is_base_type() || tt->is_enum() || tt->is_typedef()) {
    if (((t_base_type*)tt)->is_binary()) {
    ...
    out << "new " << type_name(tt, true, true) << "(" << source_name << ")";
  }
}
